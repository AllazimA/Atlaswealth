...
const ALLOWED_EMAIL = 'a.allazim@gmail.com'
    ...
    return (
      <div className="min-h-screen bg-[#111318] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-400 text-sm">This app is private. You&apos;re signed in as <span className="text-rose-400">{user.email}</span>.</p>
          <button
            onClick={() => signOut(auth)}
            className="px-4 py-2 rounded-xl border border-white/10 text-gray-300 hover:text-white hover:border-white/20 transition-colors text-sm"
          >
            Sign out
          </button>
        </div>
      ...
      <div className="min-h-screen bg-[#111318] flex items-center justify-center">
        <div className="w-full max-w-sm px-6">
          <div className="text-center mb-8">
            ...
            <h1 className="text-white text-2xl font-bold">WealthOS</h1>
            <p className="text-amber-500/70 text-sm mt-1">Premium Finance Suite</p>
          </div>

          <div className="bg-[#1a1f2e] rounded-2xl p-6 border border-white/5 space-y-4">
            <p className="text-gray-400 text-sm text-center">Sign in to access your financial dashboard</p>

            <button
              onClick={handleSignIn}
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl bg-white text-gray-900 font-semibold hover:bg-gray-100 transition-colors disabled:opacity-60"
            >
              {loading ? (
